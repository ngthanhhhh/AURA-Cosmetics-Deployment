...
API kiểm thử danh sách sản phẩm: https://aura-backend-3ep7.onrender.com/api/v1/products?page=0&size=12&sortBy=productId&direction=asc
```
...
## 11. Quy trình triển khai online

Nhóm sử dụng GitHub làm nơi quản lý source code và sử dụng nhánh `deploy` làm nhánh triển khai chính.

Khi có thay đổi cần đưa lên môi trường online, thực hiện:
...
Sau khi source code được push lên GitHub:

- Vercel tự động build lại frontend.
- Render build lại backend theo cấu hình Dockerfile đã thiết lập.
- Trong trường hợp cần thiết, backend có thể được triển khai lại thủ công trên Render bằng chức năng Manual Deploy.
